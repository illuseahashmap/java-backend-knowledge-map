...
- 减少多个线程更新同一个计数变量的竞争
- `LongAdder` 通过分散计数降低高并发竞争
- `AtomicInteger` 多线程竞争同一个值，冲突更集中
- `LongAdder` 不保证单次读取的强一致快照，不适合库存、余额等业务值
...
- 受检异常可以编译，但默认不触发回滚
- 可通过 `rollbackFor` 指定受检异常回滚
- 默认规则沿用 EJB 约定：运行时异常通常表示非预期故障，受检异常通常属于可处理的业务异常

### 事务自调用失效

- `@Transactional` 依赖 Spring 代理拦截方法调用
- 同类内部通过 `this` 调用会绕过代理，事务传播规则不会生效
...
- Dubbo 是 RPC 框架，主要用于内部微服务之间的高性能调用。
- Feign 是声明式 HTTP 客户端，主要用于调用 REST 接口。
- Dubbo 通常使用长连接和二进制协议，性能较高。
- Feign 基于 HTTP 和 JSON，通用性和跨语言能力较好。
- Dubbo 更重视服务治理，Feign 更重视 HTTP 调用的简化。

#### 核心区别

##### 通信方式

- Dubbo：RPC 协议，常使用二进制序列化
- Feign：HTTP 协议，常使用 JSON

##### 性能

- Dubbo：网络开销较小，适合高并发低延迟
- Feign：协议开销较大，但使用更加通用

##### 服务治理

- Dubbo：内置注册发现、负载均衡、路由和容错
- Feign：通常结合 Spring Cloud 组件实现服务治理

##### 跨语言

- Dubbo：依赖具体协议和语言支持
- Feign：基于 HTTP，更容易跨语言调用
...
- 跨语言服务调用：Feign
- 复杂服务治理：Dubbo
- Spring Cloud 项目：通常使用 Feign

### Eureka 和 Zookeeper 区别

#### 面试回答

- Eureka 是专门面向服务注册与发现的组件。
- Zookeeper 是分布式协调服务，也可以实现服务注册与发现。
- Eureka 采用 AP 思路，优先保证服务发现可用性。
- Zookeeper 采用 CP 思路，优先保证数据一致性。
- Eureka 更适合微服务注册中心，Zookeeper 更适合分布式协调场景。
...
- `quicklist`：双向链表连接多个紧凑存储节点
- 兼顾链表两端操作效率和内存利用率
- 普通链表节点分散，缓存局部性较差
- 节点过大：插入、删除和扩容成本高，单次延迟增加
- 节点过小：节点数量增加，指针和管理开销变大
...
- `hashtable`：通过 member 快速定位 score
...
- 快照直接读到内存
- 子进程读取 `fork` 时刻的数据视图
- 主进程修改数据时通过写时复制隔离，不影响快照内容
- 修改越多，发生写时复制的页面越多，额外内存占用越高
- 大规模数据恢复，完整性和一致性要求不高
- 宕机丢失最近一次快照
...
- 记录写指令
- 超过阈值压缩成 AOF
- 重写时只保留当前数据状态，删除重复历史命令
- 通常由 `fork` 子进程后台重写，主进程仍可处理请求
- 重写期间的新写命令暂存到重写缓冲区
- 子进程完成后，将缓冲区追加到新 AOF 文件
- 效率较低
- 不同步 / 每秒 / 每次修改三种同步策略
...
- Lua：服务端一次执行，减少网络往返和并发冲突
- WATCH：乐观锁，冲突后事务失败，需要重试
- 高并发库存扣减通常优先使用 Lua
- Redis 单线程只保证命令执行串行，不能覆盖客户端多次请求之间的间隔
- `WATCH` 用于保护读取结果到 `EXEC` 之间的数据未被修改
...
- 主节点写锁成功但未同步副本就宕机，故障转移后锁可能丢失
- 原持有者和新持有者可能同时执行，破坏互斥性

##### 解决方式

- 添加守护线程自动续期
  - 添加线程标识信息
- 释放锁时校验线程标识，再使用 Lua 删除
...
- 锁的可用时间等于 TTL 减去加锁耗时
- 多数节点失败或剩余时间不足时，需要释放已成功的节点

### Redis 的渐进式 Rehash

#### 面试回答

- Redis 扩容时不会一次性迁移全部数据。
- 它会同时维护旧表 `ht[0]` 和新表 `ht[1]`。
- 每次执行命令时迁移一部分数据，迁移期间读操作需要查询两张表。
- 全部迁移完成后，释放旧表并完成切换。
- 这样可以避免大数据量扩容时长时间阻塞 Redis。
- Redis 通过 rehashidx 记录迁移位置，每次操作在有限时间内迁移一个或多个桶；桶为空则跳过，桶不为空则迁移其中的全部节点。迁移粒度是桶，而不是固定一个 Key。
...
- 按桶迁移，Redis 命令通常由单线程串行执行，不涉及并发写入问题。
- 桶过大时，单次迁移可能产生较高延迟。
...
- 修改：检查并修改对应数据
- 删除：在对应的 Hash 表中删除
- 空闲时：Redis 也可能通过定时任务推进迁移

#### 总结

- Redis Rehash 使用两张 Hash 表
- `ht[0]` 是旧表，`ht[1]` 是新表
- 扩容后渐进式迁移数据
- 迁移期间同时查询两张表
- 迁移完成后释放旧表并切换
- 核心目的：减少扩容对 Redis 服务的阻塞
...
- 秒杀库存扣减通常需要以数据库最终结果为准
- Redis 可以用于缓存库存、限流和流量削峰
- 但只依赖 Redis 扣减库存，需要额外处理：
  - 缓存与数据库数据不一致
  - Redis 宕机或数据丢失
  - 重复扣减
  - 少卖或超卖
- 数据库扣减可靠性较高，但热点商品会导致同一行被高并发更新
- 普通 MySQL 难以承受热点行的大量并发更新
- 可以通过数据库补丁、热点行优化、分库分表、消息队列等方式提升并发能力

#### Inventory Hint

##### 使用方式

- 在普通 `UPDATE` 语句中增加特殊 Hint
...
- 当前 SQL 执行成功后，自动提交当前事务上下文
- 减少业务层显式提交事务的开销

###### `ROLLBACK_ON_FAIL`

- 当前 SQL 执行失败后，自动回滚当前事务上下文
- 保证扣减失败时事务能够回滚

###### `TARGET_AFFECT_ROW(NUMBER)`

- 要求 SQL 影响行数必须等于指定值
- 例如：`TARGET_AFFECT_ROW(1)`
- 影响 1 行表示扣减成功
- 影响 0 行通常表示库存不足或记录不存在
- 影响其他数量表示执行结果异常

##### Hint 的作用

- 告诉数据库内核当前 SQL 可能是热点更新
- 数据库内核对带有 Hint 的更新操作进行特殊处理
- 在一定时间窗口内收集并分组热点更新请求
- 根据主键或唯一键将更新相同行的请求放入同一组
- 同一组请求可以进行排队、合并和批量处理
...
- 普通更新需要通过 B+Tree 索引定位目标行
- 热点行被频繁更新时，会重复执行相同的索引查找
- Leader 首次执行时定位目标数据
- 后续请求可以复用热点行相关的数据
- 减少重复遍历 B+Tree 索引的开销
- 降低热点行更新的 CPU 和 IO 消耗

###### 减少事务提交次数

- 普通方式下，每个更新请求可能对应一次事务提交
- 高频提交会增加日志刷盘和事务管理开销
- 热点请求分组后，可以执行组提交
- 多个更新请求完成后统一提交
- 减少事务提交次数
- 降低 Redo Log 刷盘和事务提交开销

### 设计一个消息队列

#### 面试回答模板

- 我会先明确消息队列需要解决的问题，包括解耦、异步、削峰和可靠投递。
- 架构上主要包含生产者、Broker 和消费者，消息按照 Topic 分类，再通过 Queue 或 Partition 分片实现并行。
- Broker 负责消息接收、持久化、副本和投递，消费者处理成功后再确认消息。
- 可靠性方面，需要处理生产失败、Broker 宕机、消费失败和重复消费，因此会使用发送确认、持久化、多副本、重试队列、死信队列和业务幂等。
- 性能方面，可以通过分片、批量发送、批量消费和背压机制提升吞吐量。
- 如果涉及数据库事务和消息发送的一致性，可以使用本地消息表、Outbox 或事务消息。
- 最后通过消息堆积、消费延迟、失败率和副本同步延迟等指标进行监控。
...
- 我会把 RPC 框架拆成代理层、协议层、网络层、服务治理层和容错层。
- 消费者通过代理对象发起调用，框架将方法名、参数和请求 ID 编码成请求，通过长连接发送给服务提供者。
- 服务端解析请求后，根据服务名和方法名找到对应实现，执行业务逻辑并返回响应。
- 注册中心负责服务注册、服务发现和健康检查，客户端缓存服务地址并通过负载均衡选择实例。
- 可靠性方面，需要设计超时、重试、熔断、降级、故障转移和幂等。
- 性能方面，可以使用长连接、连接池、二进制序列化、异步 IO 和线程池。
- 最后通过请求 ID、Trace ID、调用耗时、失败率和超时率进行监控和链路追踪。
...
- 多个 ZSet 无法直接使用一次 `ZRANGE` 得到全局排名
- 查询时需要访问多个 ZSet
- 分别获取各分片的 Top K
- 在应用层进行归并排序
- 如果只查询全局 Top K，每个分片通常只需要获取 Top K
- 如果查询深分页，需要扩大各分片的查询范围
- 分片越多，查询和合并成本越高
...
##### 使用redis的ZRANGE + WITHSCORES

##### 避免一次性获取全部数据带来的性能和内存压力
...
- 令牌桶以固定速率生成令牌，并将令牌放入容量有限的桶中。
- 每个请求需要获取令牌，获取成功才能通过，令牌不足时进行限流。
- 令牌生成速率决定系统长期平均处理速率，桶容量决定系统允许的突发流量。
- 单机可以使用本地内存实现，分布式场景可以使用 Redis 保存令牌数量和更新时间，并通过 Lua 脚本保证扣减的原子性。
- 令牌桶相比漏桶的特点是允许一定程度的突发流量。
...
- 根据时间差计算新增令牌
- `新增令牌数 = 时间差 × 令牌生成速率`
- `当前令牌数 = min(桶容量, 原令牌数 + 新增令牌数)`

#### Redis 实现

- Redis 保存当前令牌数和上次更新时间
- 使用 Lua 脚本完成以下操作：
  - 计算新增令牌
  - 判断令牌是否足够
  - 扣减令牌
